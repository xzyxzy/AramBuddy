...
namespace AramBuddy.Plugins.KappaEvade
{
    internal static class SpellsDetector
    {
        ...
        private static void Obj_AI_Base_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (Database.SkillShotSpells.SkillShotsSpellsList.Any(s => s.hero == caster.Hero && s.slot == args.Slot
            && (args.SData.Name.Equals(s.SpellName, StringComparison.CurrentCultureIgnoreCase) || s.ExtraSpellName != null && s.ExtraSpellName.Any(x => x.Equals(args.SData.Name, StringComparison.CurrentCultureIgnoreCase)))))
            {
                //Chat.Print("OnProcessSpellCast Detected " + args.SData.Name);
                var spell = Database.SkillShotSpells.SkillShotsSpellsList.FirstOrDefault(s => s.hero == caster.Hero && s.slot == args.Slot
                && (args.SData.Name.Equals(s.SpellName, StringComparison.CurrentCultureIgnoreCase) || s.ExtraSpellName != null && s.ExtraSpellName.Any(x => x.Equals(args.SData.Name, StringComparison.CurrentCultureIgnoreCase))));
                //if (spell.DetectByMissile) return;
